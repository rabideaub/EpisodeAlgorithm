...
OTHERLUNG;
	...
	%LET CDIAG=(DIAG IN ('43301', '43311', '43321', '43331', '43381', '43391', 
					'43401', '43411', '43491')); 
...
%macro remove(ds);
	...
	%end;
%mend;
%remove(ds=R.collapsed_&CONDITION.);
/**********************************************************************************************
**********************************************************************************************/
...
data INDEX;
	...
	IF DIAG1 IN ('43301', '43311', '43321', '43331', '43381', '43391', 
					'43401', '43411', '43491') THEN SK_CASE=1;
	ELSE SK_CASE=0;
...
if (admit <= lag(disch) <= disch) and lag(provid)=provid
	and lag(hicno)=hicno
	and lag(SK_case)=SK_case=1 then combine0=1;
else combine0=0;
RUN;
...
if (admit <= lag(admit) <= disch) and 
	lag(provid)=provid
	and lag(hicno)=hicno
	and lag(SK_case)=SK_case=1 then combine=1;
else combine=0;
...
/* APPLY THE FOLLOWING INCLUSION AND EXCLUSION CRITERIA:
	SK_case=1, AGE >=65, DEAD=0, PREMO=12, POSTMOD=1, 2, 3,
...
IF (SK_case=1) AND (TRANS_COMBINE=0) AND (AGE65=1) /*AND (DEAD=0)*/					/*BR. Removed in-hospital deaths for John. 4-25-16*/
...
DATA _ALL;
MERGE ALL (IN=A WHERE=(SK_case=1)) INDEXADMISSION (IN=B KEEP=HICNO CASE);
BY HICNO CASE;
...
IF _DIAG1 IN ('43301', '43311', '43321', '43331', '43381', '43391', 
			  '43401', '43411', '43491') & _sslssnf~='N' THEN RADM_SK=1;
ELSE RADM_SK=0;  
...
if radm_rehab=1 and (radm&loe.=1 or radm&loe.p = 1) then do;  
  radm&loe.=0; radm&loe.p = 0; interval = 999; 
end;
...
/*HHA is continuous from 1996-2013. Uses CPV not CCR*/
data hha_cpv;
