...
/***********************************************************/
%let path= /schaeffer-b/sch-protected/from-projects/VERTICAL-INTEGRATION/rabideau/Programs/MainAnalysis;
%include "&path./00_Assign_Macro_Variables_and_Libraries.sas";
%include "&path./comformat2012_2015.sas"; /*Make sure the LIBRARY in this macro is the same as LIBRARY above*/

*LIBNAME IN1     '/disk/agedisk3/medicare.work/goldman-DUA25731/rabideau/Data/Vert_Int/Narrow_Networks/Final/Output';            
*LIBNAME OUT1    '/disk/agedisk3/medicare.work/goldman-DUA25731/rabideau/Data/Vert_Int/Narrow_Networks/Final/Output'; 
LIBNAME LIBRARY '/schaeffer-b/sch-protected/from-projects/VERTICAL-INTEGRATION/rabideau/Documentation'; 	   
...
/*******************************************************************/
* Maximum number of DXs;          %LET NUMDX = 25;
* Input SAS file member name;     %LET CORE  = collapsed_&DX.;
* Root name of you DIAG variables;%LET DIAG = DIAG;
* Variable name for DRG;		  %LET DRG = DRGCD;

DATA collapsed_&DX._elix;

    SET  R.&CORE (KEEP=HICNO &DRG. &DIAG.1-&DIAG.&NUMDX. /*NDX*/);

	/*Why is this even here?*/

    DROP  &DRG. &DIAG.1-&DIAG.&NUMDX. NDX I J DXVALUE A1-A30
          HTN HTNCX
   ...
   /*****************************************/ 
   ARRAY DX (&NUMDX) $  &DIAG.1 - &DIAG.&NUMDX.;
   ...
   ELSE HTN_C=0;
	...
	elix_cnt=sum(CHF,VALVE,PULMCIRC,PERIVASC,PARA,NEURO,CHRNLUNG,DM,DMCX,HYPOTHY,RENLFAIL,
				 LIVER,ULCER,AIDS,LYMPH,METS,TUMOR,ARTH,COAG,OBESE,WGHTLOSS,LYTES,BLDLOSS,
				 ANEMDEF,ALCOHOL,DRUG,PSYCH,DEPRESS);

RUN;

/*Merge the Elixhauser Scores back onto the main dataset. This is a 1:1 merge, not a match merge, so no by statement*/
data R.collapsed_&DX._elix;
	merge An_files.&CORE.
		  collapsed_&DX._elix;
run;
...
/*************************************************/  
PROC PRINT DATA=R.collapsed_&DX._elix (OBS=60);
  TITLE3 'Print of analysis file variables';
RUN;

PROC CONTENTS DATA=collapsed_&DX._elix ;
...
/****************************************/ 
PROC FREQ DATA=collapsed_&DX._elix;
...
/***********************************/  
PROC MEANS DATA=collapsed_&DX._elix  N NMISS MEAN STD MIN MAX;
